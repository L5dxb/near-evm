...
use near_primitives::views::{
    AccessKeyView, AccountView, BlockView, ExecutionErrorView,
...
impl RpcUser {
        ...
        F: FnOnce(Arc<RwLock<JsonRpcClient>>) -> Fut + 'static,
    {
        ...
        System::new("").block_on(async { f(client).await })
    }
    ...
    pub fn get_status(&self) -> Result<StatusResponse, String> {
        self.actix(move |client| client.write().unwrap().status())
...
impl User for RpcUser {
    ...
    fn get_best_block_hash(&self) -> Result<CryptoHash, String> {
        let status = self.get_status()?;
        Ok(status.sync_info.latest_block_hash.into())
    }
